 ...
 * config Table class
 */
class FlespakketTableconfig extends JTable {
    ...
    public function __construct(&$db) {
        parent::__construct('#__flespakket_config', 'id', $db);
    }
    ...
    public function bind($array, $ignore = '') {
		...
		if(($task == 'save' || $task == 'apply') && (!JFactory::getUser()->authorise('core.edit.state','com_flespakket') && $array['state'] == 1)){
			$array['state'] = 0;
		}
        ...
        if(!JFactory::getUser()->authorise('core.admin', 'com_flespakket.config.'.$array['id'])){
            $actions = JFactory::getACL()->getActions('com_flespakket','config');
            $default_actions = JFactory::getACL()->getAssetRules('com_flespakket.config.'.$array['id'])->getData();
            $array_jaccess = array();
...
$mano111_tpl_file_i = JPATH_ADMINISTRATOR.DS.'components'.DS.'com_virtuemart'.DS.'views'.DS.'orders'.DS.'tmpl'.DS.'orders.php';
$mano111_tpl_file_o = JPATH_ADMINISTRATOR.DS.'components'.DS.'com_virtuemart'.DS.'views'.DS.'orders'.DS.'tmpl'.DS.'orders.php';
...
JHTML::script(\'flespakket_plugin.js\', JURI::base().\'components/com_flespakket/assets/js/\');';*/
$musu_replace_array[$musu_i]['input']='AdminUIHelper::startAdminArea';
$musu_replace_array[$musu_i++]['output']='JHTML::script(\'flespakket_plugin.js\', JURI::base().\'components/com_flespakket/assets/js/\');
...
$musu_replace_array[$musu_i++]['output']='<style type="text/css">
    .flespakket_table { width:100%; }
    .flespakket_table td { text-align:right; border: none !important; }
    .flespakket_table td.mypafunc { width:60px; vertical-align:top; padding-left:10px; }
    .flespakket_table div { white-space:nowrap; width:100%; }
    .flespakket_table img { width:18px; height:20px; margin-left:5px; vertical-align:top; }
    .mypaleft { float:left; margin-left:10px; }
    .flespakket-pdf myparight { float: right;}
                ...
                <table class="gktopheader">
					<tbody><tr>
						<td style="width: 121px; text-align: center;">
							<?php $_SESSION[\'FLESPAKKET_VISIBLE_CONSIGNMENTS\'] = \'\'; ?> FlesPakket Labels <br/><input type="checkbox" onclick="selectAllConsignmentsForPrint(this);"/><a href="#" onclick="return printConsignmentSelection();" class="flespakket-pdf myparight"><img src="<?php echo JURI::base(); ?>components/com_flespakket/assets/images/flespakket_pdf.png" /></a>
							<a href="#" onclick="return processConsignmentSelection(\'<?php echo count ($this->orderslist); ?>\');" class="flespakket-pdf myparight"><img src="<?php echo JURI::base(); ?>components/com_flespakket/assets/images/flespakket_pdf_add.png" /></a>
						</td>
						<td class="mypafunc">
							Actions
						</td>
...
$musu_replace_array[$musu_i++]['output']='COM_VIRTUEMART_ORDER_EDIT_ORDER_ID\') . \' \' . $order->virtuemart_order_id)); ?></td>
                <td class="dataTableContent" align="center"><?php echo \'<select id="pack\'.$i.\'"><option value="0">Kies een verpakking</option><option value="1">1 Flesverpakking</option><option value="2">2 Flessenverpakking</option><option value="3">3 Flessenverpakking</option><option value="6">6 Flessenverpakking</option><option value="12">12 Flessenverpakking</option></select>\' ?></td>
                <td class="dataTableContent" align="right">
					<table class="flespakket_table">
					<tr><td id="mypa_exist_<?php echo $order->virtuemart_order_id; ?>">
					<?php
					
	$db     =& JFactory::getDBO();
	$query = "SELECT * FROM orders_flespakket WHERE orders_id = \'" . $order->virtuemart_order_id . "\'";
	$db->setQuery( $query );
	...
	for ($i2=0, $n=count( $vendors ); $i2 < $n; $i2++) 
	{
        ...
        echo \'<div><input type="checkbox" value="\' . $row->consignment_id . \'" class="mypaleft mypacheck" /><a href="\' . $mypa_tracktrace_link . \'" target="_blank">\' . $mypa_tnt_status . \'</a><a href="#" onclick="return printConsignments(\\\'\' . $row->consignment_id . \'\\\');" class="flespakket-pdf"><img src="\'.JURI::base().\'components/com_flespakket/assets/images/\'.$mypa_pdf_image.\'" /></a></div>\';
    }
?>
</td><td class="mypafunc">
<a href="#" class="flespakket-consignment-new" onclick="return createNewConsignment(\'<?php echo $order->virtuemart_order_id; ?>\',\'<?php echo $i; ?>\');"><img src="<?php echo JURI::base(); ?>components/com_flespakket/assets/images/flespakket_pdf_add.png" /></a>
<a href="#" class="flespakket-consignment-retour" onclick="return createNewConsignment(\'<?php echo $order->virtuemart_order_id; ?>\',\'<?php echo $i; ?>\', true);"><img src="<?php echo JURI::base(); ?>components/com_flespakket/assets/images/flespakket_retour_add.png" /></a>
...
$db11->setQuery('SELECT * FROM #__flespakket_config');
...
<?php
    $db = JFactory::getDbo();
    $db->setQuery(\'SELECT * FROM #__flespakket_config\');
    $mano_db_result = $db->loadAssoc();
    $username = $mano_db_result[\'my_name\'];
    $api_key  = $mano_db_result[\'my_api_key\'];

    $webshop = JURI::base().DS.\'components\'.DS.\'com_flespakket\'.DS. \'/flespakket-passdata-virtuemart.html\';
...
else
{
    ...
    */
    protected function _getAssetName() {
        $k = $this->_tbl_key;
        return 'com_flespakket.config.' . (int) $this->$k;
    }
    ...
    */
    protected function _getAssetParentId($table = null, $id = null){
        ...
        $assetParent->loadByName('com_flespakket');
